...
module.exports = {
  name: 'setautorole', description: 'Rôle automatique à l\'arrivée', category: 'config', usage: '<rôle|off>', permLevel: 'admin',
  botPermissions: [PermissionFlagsBits.ManageRoles],
  async execute(client, message, args) {
    if (!args[0]) return message.reply({ embeds: [error('Usage : `+setautorole @Membre` / `off`')] });
    ...
    if (role.managed || role.position >= message.guild.members.me.roles.highest.position) {
      return message.reply({ embeds: [error('Je ne peux pas donner ce rôle.')] });
    }
