  ...
  async execute(client, message, args) {
    ...
    if (!member && args[0]) {
      ...
      try {
        ...
        if (user) {
          await dmSanction(user, message.guild, { action: 'Ban', reason, caseId });
        }
        return message.reply({
          embeds: [success(`\`${id}\` banned. Case #${caseId}\n**Reason:** ${reason}`)],
        });
      } catch {
    ...
    if (!member.bannable) {
      return message.reply({ embeds: [error('I cannot ban this member.')] });
    }
    ...
    return message.reply({
      embeds: [
        success(`**${member.user.tag}** banned. Case #${caseId}\n**Reason:** ${reason}`),
      ],
    });
  },
};
