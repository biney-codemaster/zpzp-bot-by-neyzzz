...
async function endGiveaway(client, messageId) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `**Lot :** ${giveaway.prize}\n**Gagnant(s) :** ${
        winners.length ? winners.map((id) => `<@${id}>`).join(', ') : 'Personne (aucune participation)'
  ...
  if (winners.length) {
    await channel
      .send({
        content: `🎉 Félicitations ${winners.map((id) => `<@${id}>`).join(', ')} ! Vous gagnez **${giveaway.prize}** !`,
      })
      .catch(() => null);
  } else {
    await channel
      .send({ content: `❌ Giveaway **${giveaway.prize}** terminé sans participants.` })
      .catch(() => null);
  }
}
